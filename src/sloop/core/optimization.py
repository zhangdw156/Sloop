...
"""
                ...
                #  解析强模型的修正结果
                corrected_label = json.loads(response.choices[0].message.content.strip())
                # 更新数据
    ...
    def perform_edde(self, boundary_cases: List[Dict[str, Any]], service_definitions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for case in tqdm(boundary_cases, desc="扩展错题"):
            try:
                ...
                service = next((s for s in service_definitions if s["name"] == service_name), None)
                if not service:
...
"""
    ...
    def optimize_dataset(self, original_dataset_file: str, boundary_cases_file: str, services_file: str, output_file: str) -> None:
        """
        执行完整的优化流程：JGLV + EDDE
        ...
        """
        # 1. 加载数据
        dataset, boundary_cases = self.load_data(original_dataset_file, boundary_cases_file)
        ...
        print(f"数据集优化完成，共 {len(final_dataset)} 个样本 (原始 {len(corrected_dataset)}, 扩展 {len(expanded_dataset)})，已保存至 {output_file}")
