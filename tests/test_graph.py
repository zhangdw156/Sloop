...
def test_are_related_categories(mock_embedding):
    ...
    # general类别
    assert not builder._are_related_categories("general", "finance")
    assert not builder._are_related_categories("finance", "general")

    # 不同类别 - 使用mock向量，应该有相似度
    # 由于mock向量相同，相似度为1.0，高于阈值0.7
