...
Example: {"valid_ids": [1, 3, 5]}"""
    ...
    def build(self, 
        ...
        logger.info(f"Filtering candidates (Top-{top_k} per param & > {recall_threshold})...")
        ...
        logger.info(f"Reduced candidates to {len(rows)} edges using Top-{top_k} strategy.")
        ...
        for r, c in zip(rows, cols):
            ...
            if producer_name == consumer_name: continue
        ...
        if candidates_to_verify:
            ...
            logger.info(f"Verifying {len(candidates_to_verify)} ambiguous edges with LLM (Batch size: {batch_size})...")
            
            with tqdm(total=len(candidates_to_verify), desc="LLM Verification", unit="edge") as pbar:
        ...
        logger.info(f"Stats: High Conf (Direct): {stats['direct']}, LLM Verified: {stats['verified']}, LLM Rejected: {stats['rejected']}")
    ...
    def visualize(self, output_path: str = "data/tool_graph_vis.png"):
        ...
        nx.draw_networkx_nodes(self.graph, pos, node_size=800, node_color='#a8d5e2', alpha=0.9)
        nx.draw_networkx_edges(self.graph, pos, edge_color='gray', alpha=0.4, arrowsize=10)
    ...
    def prune_graph(self, min_component_size: int = 2):
        ...
        if nodes_to_remove:
            ...
            logger.info(f"Pruned graph: Removed {len(nodes_to_remove)} nodes belonging to components size < {min_component_size}.")
            logger.info(f"Node count: {original_count} -> {self.graph.number_of_nodes()}")
        else:
    ...
    def show(self):
        ...
        # --- Build Table ---
        table = Table(title="Semantic Tool Graph Stats", show_header=True, header_style="bold magenta")
