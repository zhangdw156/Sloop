...
# =========================================================================
    ...
    # 目前采样器仅实现了 "Sequential Chain" (线性链) 模式。
    # 为了提升 Agent 的并发处理能力和响应速度，未来建议实现以下两种并行模式：
    #
    # 1. 异构并行 (Heterogeneous Parallel) - "单实体，多维度"
    #    ----------------------------------------------------------------------
    #    * 场景: 用户针对同一个实体，需要跨工具获取不同维度的信息。
    #      (例: "帮我查一下 Google (实体) 的股价，顺便看看它的最新新闻。")
    #    * 实现思路: 
    #      - 需要构建 `param_inverted_index` (参数倒排索引: param -> [tool_list])。
    #      - 识别 "语义丛" (Semantic Bundle): 共享同一参数且动词类型安全(Read-Only)的工具集合。
    #      - 随机选取 2-3 个共享参数的工具作为无依赖的并行起点。
    #
    # 2. 同构并行 (Homogeneous/Batch Parallel) - "单工具，多实体"
    #    ----------------------------------------------------------------------
    #    * 场景: 用户在一个指令中包含了一个列表，需要批量调用同一个工具。
    #      (例: "帮我对比一下 杭州、苏州、南京 (实体列表) 的天气。")
    #    * 实现思路:
    #      - 不需要复杂的图游走。
    #      - 随机选择一个具有可枚举参数 (如 city, ticker, username) 的工具。
    #      - 生成 Blueprint 时标记 pattern="batch"，并指定 batch_size (如 3)。
    #      - 在 DataFactory 阶段，提示 LLM 编造对应数量的实体值。
    # =========================================================================