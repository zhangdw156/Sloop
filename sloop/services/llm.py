...
class LLMService:
    def __init__(self):
        ...
        self.model_name = env_config.get("OPENAI_MODEL_NAME", "Qwen3-235B-A22B-Instruct-2507")

        if not self.base_url or not self.api_key:
            logger.warning("Missing LLM configuration in .env. LLM features will be disabled.")
            self.client = None
        else:
            try:
                self.client = OpenAI(
