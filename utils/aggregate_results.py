...
CATEGORY_CONFIG = {
    ...
    # 2. Overall (原有的)
    "overall": {
        "filename": "data_overall.csv",
        "sort_col": "Overall Acc"
    },
    # 3. Live (新增)
    "live": {
        "filename": "data_live.csv",
        "sort_col": "Live Overall Acc" 
    },
    # 4. Non Live (新增)
    "non_live": {
        "filename": "data_non_live.csv",
        "sort_col": "Non Live Overall Acc"
    },
    # 5. Agentic (新增)
    "agentic": {
        "filename": "data_agentic.csv",
        "sort_col": "Agentic Overall Acc"
    },
...
def aggregate_logic(root_dir: str, output_file: str, category: str):
    ...
    if category not in CATEGORY_CONFIG:
        print(f"❌ Error: Unknown category '{category}'. Supported: {list(CATEGORY_CONFIG.keys())}")
        return
    ...
    try:
        ...
        if acc_col_name in final_header:
            ...
            if rank_col_name in final_header:
                ...
                print(f"ℹ️  'Rank' column not found in source CSV. Skipping re-ranking.")
        else:
    ...
    try:
        ...
        if len(final_header) > 3:
            try:
                # Dynamic Preview
                model_idx = final_header.index("Model") if "Model" in final_header else 2
                acc_idx = final_header.index(acc_col_name) if acc_col_name in final_header else 3
                
                print("\n--- Preview (Top 5) ---")
                # Handle cases where columns might be missing in preview
                h_model = final_header[model_idx] if model_idx < len(final_header) else "Model"
                h_acc = final_header[acc_idx] if acc_idx < len(final_header) else "Score"
...
def main(
    ...
    category: str = typer.Option("multi_turn", "--category", "-c", help=f"Category to aggregate. Options: {', '.join(CATEGORY_CONFIG.keys())}")
):
    """
