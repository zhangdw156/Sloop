...
from sloop.utils import GraphBuilder, IntentGenerator, logger, setup_logging
...
def save_intents(intents: List[UserIntent], filename: str):
    ...
    # Pydantic 转 Dict
    data_to_save = [
        intent.model_dump(by_alias=True, exclude_none=True) 
...
def main():
    ...
    if not builder.load_checkpoint(GRAPH_CHECKPOINT):
        logger.error("Please run graph_builder_exp.py first to generate the checkpoint.")
        return
    ...
    if generated_intents:
        ...
        logger.success(f"Successfully generated {len(generated_intents)}/{len(skeletons)} intents.")
    else:
